...
import processing
from qgis2web.mapboxStyleScripts import getLayerStyle
from qgis2web.mapboxScriptStrings import (popupScript,
                                          popFuncsScript,
...
def writeVectorLayer(layer, safeLayerName, usedFields, highlight,
    ...
    if is25d(layer, canvas, restrictToExtent, extent):
        ...
        for count, mblayer in enumerate(mblayers):
            ...
            layout = ""
            filter = ""
            name = "%s_%d" % (safeLayerName, count)
            type = mblayer[0]["type"]
            try:
                filter = '"filter": %s,' % mblayer[0]["filter"]
            except:
                pass
...
""" % (name, type, safeLayerName, filter, layout, paint))
